...
class WeaponManager:
    ...
    def add_ammo(self, amount):
        ...
        if self.assault.available and self.assault.ammo < self.assault.max_ammo:
            self.assault.ammo = min(self.assault.ammo + amount * 2, self.assault.max_ammo)
            received = True
        return received
    ...
    def _fire(self, spread):
        from flgame.objects.dynamic_objects.projectiles.player_projectile import PlayerProjectile
        player = self.context.player
        velocity = Vector2(_PROJECTILE_SPEED, 0).rotated(player.rotation + spread)
        self.context.world.add(PlayerProjectile(self.context, self.current.damage, player.position.copy(), velocity))
