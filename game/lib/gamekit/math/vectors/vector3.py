...
if TYPE_CHECKING:
    ...
    from gamekit.math.vectors.vector3i import Vector3i
...
class Vector3:
    ...
    def abs(self) -> "Vector3": return Vector3(abs(self.x), abs(self.y), abs(self.z))

    def angle_to(self, to: "Vector3") -> float: return math.atan2(self.cross(to).length(), self.dot(to))
    ...
    def bounce(self, n: "Vector3") -> "Vector3": return -self.reflect(n)

    def ceil(self) -> "Vector3": return Vector3(math.ceil(self.x), math.ceil(self.y), math.ceil(self.z))
    ...
    def clampf(self, min: float, max: float) -> "Vector3": return Vector3(clampf(self.x, min, max), clampf(self.y, min, max), clampf(self.z, min, max))
    ...
    def decay(self, to: "Vector3", decay: float, delta: float) -> "Vector3": return self.lerp(to, decay_weight(decay, delta))

    def direction_to(self, to: "Vector3") -> "Vector3": return Vector3(to.x - self.x, to.y - self.y, to.z - self.z).normalized()
    ...
    def distance_to(self, to: "Vector3") -> float: return math.sqrt(self.distance_squared_to(to))

    def dot(self, w: "Vector3") -> float: return self.x * w.x + self.y * w.y + self.z * w.z

    def exp(self) -> "Vector3": return Vector3(math.exp(self.x), math.exp(self.y), math.exp(self.z))

    def floor(self) -> "Vector3": return Vector3(math.floor(self.x), math.floor(self.y), math.floor(self.z))

    def inverse(self) -> "Vector3": return Vector3(inverse(self.x), inverse(self.y), inverse(self.z))
    ...
    def is_finite(self) -> bool: return math.isfinite(self.x) and math.isfinite(self.y) and math.isfinite(self.z)

    def is_normalized(self) -> bool: return abs(self.length_squared() - 1.0) < UNIT_EPSILON

    def is_zero_approx(self) -> bool: return is_zero_approx(self.x) and is_zero_approx(self.y) and is_zero_approx(self.z)

    def length(self) -> float: return math.sqrt(self.x * self.x + self.y * self.y + self.z * self.z)

    def length_squared(self) -> float: return self.x * self.x + self.y * self.y + self.z * self.z
    ...
    def max(self, w: "Vector3") -> "Vector3": return Vector3(max(self.x, w.x), max(self.y, w.y), max(self.z, w.z))

    def maxf(self, w: float) -> "Vector3": return Vector3(max(self.x, w), max(self.y, w), max(self.z, w))
    ...
    def min(self, w: "Vector3") -> "Vector3": return Vector3(min(self.x, w.x), min(self.y, w.y), min(self.z, w.z))

    def minf(self, w: float) -> "Vector3": return Vector3(min(self.x, w), min(self.y, w), min(self.z, w))
    ...
    def move_toward(self, to: "Vector3", delta: float) -> "Vector3":
        ...
        return Vector3(self.x + dx / length * delta,
                       self.y + dy / length * delta,
                       self.z + dz / length * delta)
    ...
    def posmod(self, mod: float) -> "Vector3": return Vector3(fposmod(self.x, mod), fposmod(self.y, mod), fposmod(self.z, mod))

    def posmodv(self, modv: "Vector3") -> "Vector3": return Vector3(fposmod(self.x, modv.x), fposmod(self.y, modv.y), fposmod(self.z, modv.z))

    def project(self, b: "Vector3") -> "Vector3": return b * (self.dot(b) / b.length_squared())

    def reflect(self, n: "Vector3") -> "Vector3": return 2.0 * n * self.dot(n) - self
    ...
    def round(self) -> "Vector3": return Vector3(round_half_away(self.x), round_half_away(self.y), round_half_away(self.z))

    def sign(self) -> "Vector3": return Vector3(signf(self.x), signf(self.y), signf(self.z))
    ...
    def slide(self, n: "Vector3") -> "Vector3": return self - n * self.dot(n)

    def snapped(self, step: "Vector3") -> "Vector3": return Vector3(snapped(self.x, step.x), snapped(self.y, step.y), snapped(self.z, step.z))

    def snappedf(self, step: float) -> "Vector3": return Vector3(snapped(self.x, step), snapped(self.y, step), snapped(self.z, step))

    def __add__(self, o: "Vector3") -> "Vector3": return Vector3(self.x + o.x, self.y + o.y, self.z + o.z)

    def __sub__(self, o: "Vector3") -> "Vector3": return Vector3(self.x - o.x, self.y - o.y, self.z - o.z)
    ...
    def __rmul__(self, o: float) -> "Vector3": return Vector3(self.x * o, self.y * o, self.z * o)
    ...
    def __iadd__(self, o: "Vector3") -> "Vector3":
        self.x += o.x
        self.y += o.y
        self.z += o.z
        return self

    def __isub__(self, o: "Vector3") -> "Vector3":
        self.x -= o.x
        self.y -= o.y
        self.z -= o.z
        return self

    def __imul__(self, o: "Vector3 | float") -> "Vector3":
        if isinstance(o, Vector3):
            self.x *= o.x
            self.y *= o.y
            self.z *= o.z
    ...
    def __itruediv__(self, o: "Vector3 | float") -> "Vector3":
        if isinstance(o, Vector3):
            self.x /= o.x
            self.y /= o.y
            self.z /= o.z
    ...
    def __eq__(self, o: object) -> bool: return isinstance(o, Vector3) and self.x == o.x and self.y == o.y and self.z == o.z

    def __ne__(self, o: object) -> bool: return not (isinstance(o, Vector3) and self.x == o.x and self.y == o.y and self.z == o.z)
    ...
    def __bool__(self) -> bool: return self.x != 0.0 or self.y != 0.0 or self.z != 0.0

    def __repr__(self) -> str: return "Vector3({}, {}, {})".format(self.x, self.y, self.z)
