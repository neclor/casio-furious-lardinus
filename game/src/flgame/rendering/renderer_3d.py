...
_MIN_RAY_COUNT = 64
_MAX_RAY_COUNT = Settings.resolution[0]

class Renderer3D:
    __slots__ = ('context', '_ray_count', '_ray_step_angle', '_ray_step_angle_tan', '_adjust_ray_counter', '_position', '_rotation', '_rotation_tan', '_camera_position_z')
    ...
    def _adjust_ray_count(self):
        ...
        if Settings.current_fps < Settings.fps and self._ray_count > _MIN_RAY_COUNT:
            self._ray_count -= 8
        elif Settings.current_fps > Settings.max_fps_limit and self._ray_count < _MAX_RAY_COUNT:
            self._ray_count += 8
        else:
            return
        self._update_ray_parameters()
    ...
    def _draw_floor(self):
        services.renderer.draw_rect(Rect2(0, Settings.half_resolution[1], Settings.resolution[0], Settings.half_resolution[1]), self.context.level.floor_color, z=0)

    def _draw_game(self):
        projections = self._object_projections() + self._tile_map_projections()
    ...
    def _object_projection(self, game_object):
        if game_object.sprite is None:
            return None
        radius = game_object.radius
        relative = Vector2(game_object.position - self._position).rotated(-self._rotation - Settings.HALF_PI)
        relative.y *= -1
        if relative.y <= 1:
            return None
        tan_left = (relative.x - radius) / relative.y
        tan_right = (relative.x + radius) / relative.y
        if max(tan_left, tan_right) <= -Settings.tan_half_fov_h or min(tan_left, tan_right) >= Settings.tan_half_fov_h:
            return None
        relative_bottom = game_object.position_z - self._camera_position_z
        relative_top = relative_bottom - game_object.height
        return self._calculate_projection(tan_left, tan_right, relative.y, relative_top, relative_bottom, game_object.sprite)

    def _tile_map_projections(self):
        result = []
    ...
    def _cast_ray(self, ray_rotation):
        ...
        ray_sign = Vector2((Settings.THREE_HALF_PI < ray_rotation <= 2 * math.pi or 0 <= ray_rotation < Settings.HALF_PI) - (Settings.HALF_PI < ray_rotation < Settings.THREE_HALF_PI), (0 < ray_rotation < math.pi) - (math.pi < ray_rotation < 2 * math.pi))
        signed_tile_size = Vector2(ray_sign.x * tile_size.x, ray_sign.y * tile_size.y)
        ray_tan = math.tan(ray_rotation)
        abs_ray_tan = abs(ray_tan)
        ray_relative_angle_cos = math.cos(ray_relative_angle)
        ray_relative_angle_tan = (ray_tan - self._rotation_tan) / (1 + ray_tan * self._rotation_tan)
        right_ray_relative_angle_tan = (ray_relative_angle_tan + self._ray_step_angle_tan) / (1 - ray_relative_angle_tan * self._ray_step_angle_tan)
        ...
        tile_index = Vector2(x_whole - (ray_sign.x < 0 and x_fractional == 0), y_whole - (ray_sign.y < 0 and y_fractional == 0))
        next_line = Vector2((tile_index.x + (0 <= ray_sign.x)) * tile_size.x, (tile_index.y + (0 <= ray_sign.y)) * tile_size.y)
        ...
        while not self._line_out_of_bounds(ray_sign, tile_index, map_size):
            ...
            delta_ratio = delta_next_line.y / delta_next_line.x if delta_next_line.x != 0 else float('inf')
            tan_delta_difference = abs_ray_tan - abs(delta_ratio)
            if tan_delta_difference < 0:
                ray_position = Vector2(next_line.x, position.y + (next_line.x - position.x) * ray_tan)
                next_line.x += signed_tile_size.x
                tile_index.x += ray_sign.x
            elif tan_delta_difference > 0:
                ray_position = Vector2(position.x + (next_line.y - position.y) / ray_tan, next_line.y)
            ...
            if relative_min_obscured_point <= level.min_point_z - camera_z and level.max_point_z - camera_z <= relative_max_obscured_point:
                break
            if not (0 <= tile_index.x < map_size.x and 0 <= tile_index.y < map_size.y):
                continue
            ...
            if not (min_tile_point_visible or max_tile_point_visible):
                continue
            ...
            texture_offset_x = abs(ray_position.x % tile_size.x / tile_size.x - max(ray_sign.y, 0)) % 1
            texture_offset_y = abs(ray_position.y % tile_size.y / tile_size.y + min(ray_sign.x, 0)) % 1
            x_offset_larger_y_offset = texture_offset_x > texture_offset_y
            texture_offset = texture_offset_x if x_offset_larger_y_offset else texture_offset_y
            texture = tile.texture
            texture_x = min(int(texture_offset * texture.width), texture.width - 1)
            column_src = Rect2i(texture_x, 0, 1, texture.height)
            tile_projections.append(self._calculate_projection(ray_relative_angle_tan, right_ray_relative_angle_tan, distance * ray_relative_angle_cos, relative_tile_top, relative_tile_bottom, texture, column_src))
        return tile_projections

    @staticmethod
    def _line_out_of_bounds(ray_sign, tile_index, map_size):
        return ray_sign.x < 0 and tile_index.x < 0 or (ray_sign.y < 0 and tile_index.y < 0) or (0 < ray_sign.x and map_size.x <= tile_index.x) or (0 < ray_sign.y and map_size.y <= tile_index.y)

    @staticmethod
    def _calculate_projection(left_tan, right_tan, distance, relative_top, relative_bottom, texture, src=None):
        ...
        if distance >= 1:
            ...
            height = relative_bottom * y_factor + Settings.half_resolution[1] - position_y
        return (texture, src, Rect2(math.floor(position_x), math.floor(position_y), math.ceil(width), math.ceil(height)), flip_v, distance)
