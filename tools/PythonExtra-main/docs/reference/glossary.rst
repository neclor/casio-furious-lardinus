...
.. glossary::
    ...
    baremetal
        ...
        PythonExtra effectively functions like a small operating system,
    ...
    board
        ...
        PythonExtra firmware is typically provided per-board, as the firmware
    ...
    bytecode
        ...
        PythonExtra both use bytecode, the format is different. You can also
        pre-compile source code offline using the :term:`cross-compiler`.

    callee-owned tuple
        This is a PythonExtra-specific construct where, for efficiency
    ...
    CircuitPython
        A variant of PythonExtra developed by `Adafruit Industries
    ...
    CPython
        ...
        implementations (including Jython, IronPython, PyPy, and PythonExtra).
        While PythonExtra's implementation differs substantially from CPython,
        it aims to maintain as much compatibility as possible.

    cross-compiler
        Also known as ``mpy-cross``. This tool runs on your PC and converts a
        :term:`.py file` containing PythonExtra code into a :term:`.mpy file`
        containing PythonExtra :term:`bytecode`. This means it loads faster (the board
        doesn't have to compile the code), and uses less space on flash (the
        bytecode is more space efficient).

    driver
        A PythonExtra library that implements support for a particular
        component, such as a sensor or display.

    FFI
        Acronym for Foreign Function Interface. A mechanism used by the
        :term:`PythonExtra Unix port` to access operating system functionality.
        This is not available on :term:`baremetal` ports.

    filesystem
        Most PythonExtra ports and boards provide a filesystem stored in flash
    ...
    Garbage Collector
        A background process that runs in Python (and PythonExtra) to reclaim
    ...
    GPIO
        ...
        their digital value (logical "0" or "1"). PythonExtra abstracts GPIO
    ...
    heap
        A region of RAM where PythonExtra stores dynamic data. It is managed
    ...
    interned string
        An optimisation used by PythonExtra to improve the efficiency of
    ...
    MCU
        ...
        require much less power. PythonExtra is designed to be small and
        optimized enough to run on an average modern microcontroller.

    micropython-lib
        PythonExtra is (usually) distributed as a single executable/binary
        ...
        only be used on the :term:`PythonExtra Unix port` (with limited support
        for Windows).
    ...
    PythonExtra port
        PythonExtra supports different :term:`boards <board>`, RTOSes, and
        OSes, and can be relatively easily adapted to new systems. PythonExtra
        ...
        available across different ports ("PythonExtra core"). Note that some
        ...
        beyond the PythonExtra core functionality, would be described in the
        separate port-specific documentation.

    PythonExtra Unix port
        The unix port is one of the major :term:`PythonExtra ports
        <PythonExtra port>`. It is intended to run on POSIX-compatible
        ...
        quick development and testing of the PythonExtra language and
        machine-independent features. It can also function in a similar way to
        :term:`CPython`'s ``python`` executable.

    mip
        A package installer for PythonExtra (mip - "mip installs packages"). It
        installs PythonExtra packages either from :term:`micropython-lib`,
    ...
    REPL
        ...
        Most PythonExtra boards make a REPL available over a UART, and this is
    ...
    stream
        ...
        ``close()``, etc. A stream is an important concept in PythonExtra;
        ...
        information on streams in PythonExtra, see the `io` module.
    ...
    upip
        A now-obsolete package manager for PythonExtra, inspired
