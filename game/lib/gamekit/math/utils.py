...
def signf(v: float) -> float: return float((v > 0.0) - (v < 0.0))

def signi(v: int) -> int: return (v > 0) - (v < 0)

def lerp(a: float, b: float, t: float) -> float: return a + (b - a) * t

def lerpi(a: int, b: int, t: float) -> int: return int(a + (b - a) * t)

def decay_weight(decay: float, delta: float) -> float: return 1.0 - math.exp(-decay * delta)

def clampf(v: float, lo: float, hi: float) -> float: return lo if v < lo else (hi if v > hi else v)

def clampi(v: int, lo: int, hi: int) -> int: return lo if v < lo else (hi if v > hi else v)

def round_half_away(v: float) -> float: return float(math.floor(v + 0.5)) if v >= 0.0 else float(math.ceil(v - 0.5))

def inverse(v: float) -> float: return float("inf") if v == 0.0 else 1.0 / v
...
def snapped(value: float, step: float) -> float:
    if step != 0.0: return math.floor(value / step + 0.5) * step

    return value


def snappedi(value: int, step: int) -> int:
    if step != 0: return int(math.floor(value / step + 0.5)) * step
...
def is_zero_approx(a: float) -> bool: return abs(a) < CMP_EPSILON
